...
function page13Animation() {
  // gsap.set(".department1", {opacity: 0});
  gsap.from(".department1", {
    x: -250,
    opacity: 0,
    duration: 0.75,
    ease: "power3.out",
...
function animateValveGuide() {
  gsap.from(".valve-guide > *", {
    y: 50,
    opacity: 0,
    duration: 0.75,
    ease: "power3.out",
...
function updatePageNumber() {
  ...
  if (!(currentSlide + 1 == 34)) {closeAllFaqs();}
...
function playAudioForPage(pageNumber) {
  ...
  const pagesWithoutAudio = [2, 26, 56];
...
pageInput.addEventListener("keydown", (e) => {
  if (e.key === "Enter") {
    ...
    if (!isNaN(pageNumber) && pageNumber >= 1 && pageNumber <= totalSlides) {
      ...
      pageInput.value = currentSlide + 1; // Reset to current if invalid
    }
  }
});
...
function animateSwiperContainer7() {
  gsap.from(".swiper-container7 .content", {
    x: -250,       // start 100px to the right
    opacity: 0,
    duration: 0.75,
    ease: "power3.out",
    // stagger: 0.15
...
function updateSlideContent(swiperInstance) {
  ...
  document.querySelectorAll(".venting-swiper4new .reveal").forEach((el) => {
    el.classList.remove("revealed");
  });

  const activeSlide = swiperInstance.slides[swiperInstance.activeIndex];
  const revealEl = activeSlide.querySelector(".venting-swiper4new .reveal");
...
function revealOnlyActiveSlide(swiper) {
  swiper.slides.forEach((slide, index) => {
    const reveal = slide.querySelector(".venting-swiper11 .reveal");
    if (reveal) {
      if (index === swiper.activeIndex) {
...
function toggleHotspots(visible) {
  ...
  document.querySelector(".faq-model").style.display = visible ? "inline" : "none";
}
...
modelViewer.querySelectorAll(".Hotspot").forEach((button) => {
  ...
  // });
   button.addEventListener("click", () => {
    ...
    // ✅ Find the matching FAQ item by question text
    const matchingItem = [...document.querySelectorAll('.faq-model-item')].find(item =>
      item.querySelector('.faq-model-question')?.textContent.trim() === content.title
    );

    if (matchingItem) {
      const answer = matchingItem.querySelector('.faq-model-answer');
      const icon = matchingItem.querySelector('.chevron');
    ...
    // ❌ Do NOT show `.label-content` anymore
    // $(".label-content").fadeIn(); → REMOVE this line
  });
});
...
const faqItems = document.querySelectorAll('.faq-model-item');

  faqItems.forEach(item => {
    const question = item.querySelector('.faq-model-question');
    const answer = item.querySelector('.faq-model-answer');
  ...
  function closeAllFaqs() {
    document.querySelectorAll('.faq-model-answer').forEach(a => a.classList.remove('open'));
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  images.forEach(img => {
    // Wrap the image in a span for positioning
    const wrapper = document.createElement('span');
    wrapper.className = 'zoomable-wrapper';
    img.parentNode.insertBefore(wrapper, img);
    wrapper.appendChild(img);

    // Create and add expand icon
    const expandIcon = document.createElement('div');
    expandIcon.className = 'expand-icon';
    expandIcon.innerHTML = '&#x26F6;'; // Unicode expand icon
  ...
  // Close modal
  document.querySelector('.zoomImagemodal .close').onclick = function () {
